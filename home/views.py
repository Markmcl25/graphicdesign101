...
def shopping_bag(request):
    ...
    for item_id, quantity in bag.items():
        product = get_object_or_404(
            PortfolioProject, id=item_id)  # Get the product
...
def checkout(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            total_price = sum(PortfolioProject.objects.get(
                id=item_id).price * quantity for item_id, quantity in bag_items.items())
            ...
            try:
                ...
                messages.error(
                    request, f"Error processing payment: {e.user_message}")
        else:
...
def add_to_cart(request, project_id):
    ...
    messages.success(
        request, f'{project.title} has been added to your shopping bag!')
...
def submit_inquiry(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Redirect to the homepage or thank you page
